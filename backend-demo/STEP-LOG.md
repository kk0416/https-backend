...
1. 为下次在 `Ubuntu` 上继续发布预先整理一份可直接照着执行的文档
2. 文档覆盖：
   1. Ubuntu 本机编译验证
   2. Docker 构建
   3. SWR 推送
...
1. 新增文档：`docs/Ubuntu编译并发布到华为云步骤.md`
2. 文档中明确写入了：
   1. 当前固定镜像仓库
   2. 当前固定命名空间
   3. 当前固定域名
   4. 推荐使用 `docker buildx build --provenance=false --sbom=false --push`
   5. 推送后使用 `helm upgrade --install ... --set image.tag=${TAG}`
3. 更新了 `docs/使用说明.md`
   1. 增加 Ubuntu 发布文档入口

### 当前结果

1. 下次切到 Ubuntu 后，可以直接按新文档执行
2. 当前文档已经把这次踩过的 SWR 格式兼容坑和 Helm 更新镜像 tag 的要点一起记下来了

## 步骤 3.11 整理 Ubuntu 极简发布清单
...
1. 再提供一份只保留命令的发布清单
2. 方便下次进入 Ubuntu 后直接复制执行

### 本步操作

1. 新增文档：`docs/Ubuntu极简发布清单.md`
2. 文档中只保留了三组命令：
   1. Ubuntu 同时能访问 SWR 和 CCE
   2. Ubuntu 只能访问 SWR
   3. 回到能访问 CCE 的机器后继续发布
3. 更新了 `docs/使用说明.md`
   1. 增加极简发布清单入口

### 当前结果

1. 现在既有详细版发布文档，也有极简命令版清单
2. 下次要快速执行时，可以直接打开极简版

## 步骤 3.12 整理 Windows 手动编译与运行步骤
...
1. 单独整理一份 Windows 下手动编译和运行后端的文档
2. 方便在不使用 Docker 的情况下做本地验证

### 本步操作

1. 新增文档：`docs/Windows手动编译与运行步骤.md`
2. 文档中整理了：
   1. `npm ci`
   ...
   5. 本地 `curl` 验证命令
3. 更新了 `docs/使用说明.md`
   1. 增加 Windows 手动编译与运行文档入口

### 当前结果

1. 现在已经有单独的 Windows 手动编译说明
2. 本地不走 Docker 也可以直接按文档验证编译产物
...
1. 将“从 PVC 存储改为 OBS 存储”的最小改造范围单独整理成文档
2. 便于后续按文件逐步实施

### 本步操作

1. 新增文档：`docs/OBS最小改造清单.md`
2. 文档中整理了：
   1. 必改文件
   2. 建议新增文件
   3. Helm 相关改造点
...
1. 后续如果要从 PVC 上传方案切到 OBS，可以直接按新文档执行
2. 当前改造范围已经从“概念讨论”收敛到“逐文件修改清单”
