...
function resolveRuntimeFile(filePath) {
    ...
    return (0, node_fs_1.existsSync)(absolutePath) ? absolutePath : undefined;
}
function createHttpsOptions(logger) {
    ...
    if (pfxPath) {
        ...
        return {
            httpsOptions: {
                pfx: (0, node_fs_1.readFileSync)(pfxPath),
                passphrase: pfxPassphrase,
            },
            mode: 'pfx',
        };
    }
    throw new Error('未找到可用的 HTTPS 证书。请配置 SSL_CERT_FILE/SSL_KEY_FILE，或提供 HTTPS_PFX_PATH。');
}
async function bootstrap() {
    ...
    fastify.get('/health', async () => (0, api_response_1.ok)({
